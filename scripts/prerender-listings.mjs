...
function generateListingPage(listing, imageUrl) {
  ...
  const description = `${safeMake} ${safeModel} ${safeYear} in ${safeLocation}. ${safeCondition}.${listing.mileage ? ' ' + Number(listing.mileage).toLocaleString('en-NG') + ' km' : ''}. Price: ${safePrice}.`;
...
async function fetchAllImages(listingIds) {
  const imagesMap = {};
  const chunkSize = 25;
...
async function main() {
  ...
  const imagesMap = await fetchAllImages(ids);
